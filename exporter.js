...
function extractTokenCandidatesFromText(text) {
    ...
    // Stop at punctuation/newline/paren so we don't eat the whole paragraph.
    const attemptRe = /Attempt to\s+([^\n\r.,;:()]+?)(?=[\n\r.,;:()]|$)/gi;

    // Create a/an <Something> token
    const createRe = /Create\s+(?:a|an)\s+([^\n\r.,;:()]+?)\s+token\b/gi;

    let m;
    while ((m = attemptRe.exec(t)) !== null) out.push(m[1]);
    while ((m = createRe.exec(t)) !== null) out.push(m[1]);

    return out.map(normalizeTokenName).filter(Boolean);
}
...
function canonicalizeCardTitle(maybeTitle) {
...
function collectTokensForCurrentDeck({ recursive = true, maxScans = 5000 } = {}) {
    ...
    const titlesToScan = new Set([
        ...state.startingDeck,
        ...state.purchaseDeck,
        ...(state.selectedWrestler ? [state.selectedWrestler.title] : []),
        ...(state.selectedManager ? [state.selectedManager.title] : []),
    ...
    const scanTitle = (title) => {
        ...
        if (!card) return [];
        const text = getCardRulesText(card);
    ...
    while (queue.length) {
        ...
        const candidates = scanTitle(canonCurrent);
        for (const cand of candidates) {
            const canonToken = canonicalizeCardTitle(cand);
