...
function getBasePath() {
    ...
    if (secondSlashIndex !== -1) return path.substring(0, secondSlashIndex + 1);
    return '/';
}
...
function buildKeywordMatchers(keywordNames) {
    // Match keywords as whole words/phrases, case-insensitive.
    // This catches things like "Commit", "Relentless", etc in raw game text.
    // For multiword keywords, this still works because we wrap with boundaries.
    return keywordNames.map(name => {
        const pat = `\\b${escapeRegex(name)}\\b`;
        return { name, re: new RegExp(pat, 'i') };
    });
}
...
export async function loadGameData() {
    ...
    try {
        ...
        // Load keywords first so we can auto-derive per-card keywords from raw text
        ...
        const parsedCards = cardLines.map(line => {
            ...
            // KEYWORDS (UPDATED):
            // 1) If a Keywords column exists, use it.
            // 2) Otherwise derive from raw game text using keywords.txt list.
            const hasKeywordsColumn = Object.prototype.hasOwnProperty.call(card, 'Keywords') || Object.prototype.hasOwnProperty.call(card, 'Keyword');
            const keywordField = card.Keywords ?? card.Keyword ?? null;

            if (hasKeywordsColumn && typeof keywordField === 'string' && keywordField.trim() !== '') {
                ...
                for (const { name, re } of keywordMatchers) {
                    if (re.test(rawText)) found.push({ name });
                }
                card.text_box.keywords = found;
            }

            // Traits: keep existing behavior (still column-based if present)
            if (card.Traits) {
