...
// UPDATED: Improved text wrapping function that handles very long text better
...
// UPDATED: More aggressive text scaling for small cards
function scaleTextToFit(ctx, text, maxWidth, maxHeight, isSmallCard = false) {
    let fontSize = isSmallCard ? 12 : 20; // Start smaller for digital cards
    let lines = [];
    let totalHeight = 0;
    
    while (fontSize >= (isSmallCard ? 6 : 10)) {
        ctx.font = `${fontSize}px Arial`;
        lines = wrapText(ctx, text, maxWidth);
        
        const lineHeight = fontSize * (isSmallCard ? 1.2 : 1.4);
        totalHeight = lines.length * lineHeight;
        
        if (totalHeight <= maxHeight) {
            break;
        }
        fontSize -= (isSmallCard ? 0.5 : 1);
    }
    
    // If text still doesn't fit at minimum size, truncate
    if (fontSize === (isSmallCard ? 6 : 10) && totalHeight > maxHeight) {
        // Remove lines until it fits
        while (lines.length > 0 && (lines.length * fontSize * (isSmallCard ? 1.2 : 1.4)) > maxHeight) {
            lines.pop();
        }
        ...
        if (lines.length > 0) {
            ...
            let dotsWidth = ctx.measureText('...').width;
            ...
            if (truncatedLine.length < lastLine.length) {
                lines[lines.length - 1] = truncatedLine + '...';
            } else if (lines.length > 1) {
                // If we can't truncate enough, remove another line
                lines.pop();
                if (lines.length > 0) {
                    const newLastLine = lines[lines.length - 1];
                    let newTruncated = newLastLine;
                    while (ctx.measureText(newTruncated + '...').width > maxWidth && newTruncated.length > 3) {
                        newTruncated = newTruncated.substring(0, newTruncated.length - 1);
                    }
                    lines[lines.length - 1] = newTruncated + '...';
                }
            }
        }
    }
    ...
    return {
        fontSize,
        lines,
        lineHeight: fontSize * (isSmallCard ? 1.2 : 1.4)
    };
}
...
// UPDATED: IMPROVED RENDERER with better text scaling
function renderCardToCanvas(card, width, height) {
    ...
    if (isSmallSize) {
        // =================================================================
        // SIMPLE CLEAN LAYOUT FOR 214x308 DIGITAL CARDS - IMPROVED TEXT SCALING
        // =================================================================
        ...
        // 6. TEXT BOX AREA - INCREASED SIZE FOR SMALL CARDS
        const textBoxHeight = height - textBoxY - 10; // Reduced bottom margin
        ...
        if (rawText) {
            ...
            // Define text area with more space
            const textX = 12;
            const textY = textBoxY + 8;
            const maxTextWidth = width - 24;
            const maxTextHeight = textBoxHeight - 12;
            
            // Use the new scaling function
            const textInfo = scaleTextToFit(ctx, cleanText, maxTextWidth, maxTextHeight, true);
            
            // Render lines with proper scaling
            for (let i = 0; i < textInfo.lines.length; i++) {
                const yPos = textY + (i * textInfo.lineHeight);
                if (yPos + textInfo.lineHeight <= textBoxY + textBoxHeight - 5) {
                    // Simple text rendering for small cards
                    ctx.font = `${textInfo.fontSize}px Arial`;
                    ctx.fillText(textInfo.lines[i], textX, yPos);
                }
            }
        ...
        // =================================================================
        // LARGER CARD LAYOUT (750x1050 or 1500x2100) WITH IMPROVED TEXT SCALING
        // =================================================================
        const scale = width / 750;
        const isHighRes = width === 1500;
        ...
        const textBoxHeight = height - textBoxY - 40 * scale;
        ...
        // Card text with improved scaling
        const rawText = card.text_box?.raw_text || '';
        if (rawText) {
            const cleanText = cleanCardText(rawText);
            ctx.fillStyle = '#333';
            ...
            const maxTextHeight = textBoxHeight - 40 * scale;
            
            // Use the new scaling function
            const textInfo = scaleTextToFit(ctx, cleanText, maxTextWidth, maxTextHeight, false);
            ...
            for (let i = 0; i < textInfo.lines.length; i++) {
                const yPos = textY + (i * textInfo.lineHeight);
                if (yPos + textInfo.lineHeight <= textBoxY + textBoxHeight - 10 * scale) {
                    ctx.fillText(textInfo.lines[i], textX, yPos);
                }
            }
        }
...
export { 
    ...
    exportAllCardsAsImagesFallback,
    exportAllCardsAsTSV
};
